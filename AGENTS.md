...
- **Performance optimization**: To reduce execution time: (1) reduce max iterations for both coordinator and specialist agents, (2) shorten system prompts to be direct/concise, (3) guide coordinator to investigate fewer services (1-2 max), (4) reduce investigator iterations to minimum needed (3+ for checking multiple diagnostic sources). Hierarchical agents multiply LLM calls (coordinator × investigators); iteration count of 3 allows checking multiple tools (status, logs, metrics) while 2 is too restrictive
- **Performance optimization**: To reduce execution time: (1) reduce max iterations for both coordinator and specialist agents, (2) shorten system prompts to be direct/concise, (3) guide coordinator to investigate fewer services (1-2 max), (4) reduce investigator iterations to minimum needed (2-3). Hierarchical agents multiply LLM calls (coordinator × investigators)